...
def save_log(result):
	try:
		...
		f = open("/var/log/pi-star/GPS_NMEA_log.txt",'a')
		f.writelines("\n%s:log:%s" %(now,result))
		f.flush()
		f.close()
...
def NMEA_GPGGA(sentence):
	...
	if match:
		...
		if len(parts) > 9 and parts[2] and parts[4] and parts[9]:
			...
			lat_dir=parts[3]
			lon_dir=parts[5]
			altitude=float(parts[9]) #NMEA海拔数据单位米
			altitude=altitude*3.28 #APRS海拔数据单位英尺，米转英尺
			lat_dd="%.2f"%lat
			lon_dd="%.2f"%lon
...
def NMEA_GPRMC(sentence):
	match=re.match(r'^\$GPRMC,.*', sentence)  # 匹配GPGGA语句
	if match:
		parts=sentence.split(',')
		print(parts)
		if len(parts) > 8 and parts[7] and parts[8]:
			speed="%03.0f"%float(parts[7]) #NMEA APRS速度数据单位均为海里/小时
			course="%03.0f"%float(parts[8]) #NMEA APRS航向数据单位均为度
			return speed,course
		else:
			print("No %s Signal. Waiting....."%parts[0])
	return None,None
		...
		while True:
			if ser.in_waiting > 0:  # 检查是否有数据可读
				...
				if lat is not None and lon is not None and altitude is not None:
					print(f"GNSS GPGGA: 纬度={lat}, 经度={lon}, 海拔={altitude}")
					break
			time.sleep(0.1)
		i=0
		speed='000'
		...
		while i<60:
			if ser.in_waiting > 0:  # 检查是否有数据可读
				...
				if speed is not None and course is not None:
					print(f"GNSS GPRMC: 速度={speed}, 航向={course}")
					break
			#time.sleep(0.1)
			i+=1
